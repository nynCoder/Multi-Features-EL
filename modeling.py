        ...
        with tf.variable_scope('bert', reuse=tf.AUTO_REUSE):
            with tf.variable_scope("embeddings"):
                ...
                self.embedding_output = embedding_postprocessor(
                    ...
                    position_type_ids=input_position_ids,
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
    ...
    for x in init_vars:
        ...
        if name not in name_to_variable:
            if name.startswith('bert') and 'embeddings' not in name:
                tmp1 = name.replace('bert/', 'bert_1/')
                tmp2 = name.replace('bert/', 'bert_2/')
                if tmp1 not in name_to_variable and tmp2 not in name_to_variable: continue
                vname = []
                if tmp1 in name_to_variable: vname.append(tmp1)
                if tmp2 in name_to_variable: vname.append(tmp2)
            else: continue
        #print('1111111111111')
        assignment_map[name] = vname
        #assignment_map[name] = [name_to_variable[name]]
        for vn in vname:
            initialized_variable_names[vn] = 1
            initialized_variable_names[vn + ":0"] = 1
    ...
    if use_position_embeddings:
        ...
        with tf.control_dependencies([assert_op]):
            ...
            if position_type_ids is None:
                ...
                position_embeddings = tf.nn.embedding_lookup(full_position_embeddings, position_type_ids)
...
def layers_attention(bert_output,
                    bert_output_pool,
                    num_class_list,
                    attention_unit_size,
                    fc_hidden_size
    ...
    layer_input = bert_output
    layer_num = len(num_class_list)
    
    all_layer_attention_output = []

    for i in range(layer_num):
        layer_logits,layer_scores,layer_visual,layer_local_fc_out = single_layer_attention(layer_input,bert_output_pool,num_class_list[i],
                                                                                            attention_unit_size,fc_hidden_size,i)
        next_layer_input = tf.multiply(bert_output, tf.expand_dims(layer_visual, -1))
        layer_input = next_layer_input
        
        #all_layer_attention_logits.append(layer_logits)
        #all_layer_attention_logits.append(layer_scores)
        all_layer_attention_output.append(layer_local_fc_out)

    return all_layer_attention_output

def single_layer_attention(up_output,
                           bert_output_pool,
                           layer_class_num,
                           attention_unit_size,
                           fc_hidden_size,
    ...
    单个层级注意力
    '''
    layer_att_weight, layer_att_out = _attention(up_output, layer_class_num,attention_unit_size, name="layer_attention_%d_" % scope_name)
    layer_local_input = tf.concat([bert_output_pool, layer_att_out], axis=1)
    layer_local_fc_out = _fc_layer(layer_local_input,fc_hidden_size, name="layer_fc_local_%d_" % scope_name)
    layer_logits, layer_scores, layer_visual = _local_layer(layer_local_fc_out, layer_att_weight, 
                                                            layer_class_num, name="layer_local_%d_" % scope_name)
    return layer_logits,layer_scores,layer_visual,layer_local_fc_out


def _attention(input_x, num_classes,attention_unit_size, name=""):
     """
     Attention Layer.
...
def _fc_layer(input_x,fc_hidden_size, name=""):
    """ 
